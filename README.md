...
6. ~~Item-level Tally detail~~ — DONE (code-complete; real-data verification pending). `TallyVoucherItem` + `GET /tally-import/item-cross-check` cross-checks sqft against `sales_line_item`. No real Tally XML export exists in this repo to verify parsing against — running that verification is the Owner's own manual step.
